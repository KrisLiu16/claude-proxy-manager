...
```bash
curl -fsSL https://raw.githubusercontent.com/KrisLiu16/claude-proxy-manager/main/install.sh | CPM_VERSION=v0.2.0 sh
```
...
- SOCKS5 主机、端口、用户名和密码
- `NO_PROXY` 白名单
- Claude 进程时区，默认 `America/Los_Angeles`
- Claude 进程 locale，默认 `en_US.UTF-8`
...
时区与 locale 可以按机器修改。bridge 只监听 `127.0.0.1`，SOCKS5 凭据从权限为 `0600` 的配置文件读取，不进入进程参数。
