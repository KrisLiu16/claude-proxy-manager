...
export async function runContainerCommand(command: string, args: string[]): Promise<number> {
	...
	try {
		...
		checks.push({name: '容器普通用户', state: uid !== undefined && uid !== 0 ? 'PASS' : 'FAIL', value: String(uid ?? '<未知>')});
		checks.push({name: '独立主机名', state: hostname() === 'cpm-dev' ? 'PASS' : 'FAIL', value: hostname()});
		const interfaces = Object.values(networkInterfaces()).flat().filter((item): item is NonNullable<typeof item> => Boolean(item));
		checks.push({name: '网络接口', state: interfaces.every(item => item.internal) ? 'PASS' : 'FAIL', value: interfaces.map(item => item.address).join(',')});
		checks.push({name: '直连公网', state: await externalTcpBlocked('1.1.1.1') ? 'PASS' : 'FAIL', value: '不可达'});
		checks.push({name: '云元数据直连', state: await externalTcpBlocked('169.254.169.254') ? 'PASS' : 'FAIL', value: '不可达'});
		...
		checks.push({name: '宿主设备信息', state: (await readdir('/sys')).length === 0 ? 'PASS' : 'FAIL', value: '已遮蔽 /sys'});
		let dockerSocketVisible = false;
		try { await access('/var/run/docker.sock'); dockerSocketVisible = true; } catch {}
		checks.push({name: 'Docker 控制口', state: dockerSocketVisible ? 'FAIL' : 'PASS', value: dockerSocketVisible ? '可访问' : '不可访问'});
		checks.push({name: '代理凭据环境', state: !process.env.SOCKS5_PROXY && !process.env.CPM_PROXY_CONFIG ? 'PASS' : 'FAIL', value: '未注入'});
		const resolver = await readFile('/etc/resolv.conf', 'utf8');
		checks.push({name: 'DNS 配置', state: /^nameserver 127\.0\.0\.1$/m.test(resolver) ? 'PASS' : 'FAIL', value: '127.0.0.1'});
		const address = (await lookup('api.ipify.org', {family: 4})).address;
		checks.push({name: '隔离 DNS', state: /^198\.(18|19)\./.test(address) ? 'PASS' : 'FAIL', value: address});
		const timezone = (await readFile('/etc/timezone', 'utf8')).trim();
		checks.push({name: '系统时区', state: timezone === process.env.TZ ? 'PASS' : 'FAIL', value: timezone});
		let exitIp = '';
		try {
		...
		checks.push({name: '沙箱实际出口', state: exitIp === process.env.CPM_EXPECTED_EXIT_IP ? 'PASS' : 'FAIL', value: exitIp, detail: `预期 ${process.env.CPM_EXPECTED_EXIT_IP || '<无>'}`});
		console.error(`CPM 独立容器启动检查\n${statusSummary({connected: true, checks})}\n`);
